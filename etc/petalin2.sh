...
docker run -ti $SET_X_SERVER $SET_MIRROR_PATH -v "$PWD":"$PWD" -v "$PWD/tftpboot":/tftpboot -v ~/Projects:/app -w "$PWD" --rm -u petalinux $OVERRIDE_ENTRYPOINT docker_petalinux2:$latest $SET_DOCKER_COMMAND
